...
void load_texture(struct engine* engine) {
	LOGI("Loading png");
	...
	while ((filename = AAssetDir_getNextFileName(assetDir)) != NULL) {
		...
		//AAsset* asset = AAssetManager_open(manager, "tiles2.png", AASSET_MODE_BUFFER);

		//off_t bufferSize = 256 * 256;
		...
		//unsigned char* pdata = (unsigned char*)AAsset_read(asset, buffer, fileLength);
		...
		engine->mCubeRenderer->sendTextureToOpenGL((unsigned char*)image_data, engine->mCubeRenderer->textureVec[j]);
...
static int engine_init_display(struct engine* engine) {
	...
	* gefahrlos mithilfe von  "EGL_NATIVE_VISUAL_ID" entsprechend neu konfigurieren. */
	...
	engine->mCubeRenderer->textureVec.resize(2);
